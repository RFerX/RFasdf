...
class AutomationBotApp(ctk.CTk):
    def __init__(self):
        ...
        self.add_log("System initialized. All themes synchronized.", "SYSTEM", "blue")
        self.protocol("WM_DELETE_WINDOW", self.on_closing)

    # --- LOGIKA SESSION (AUTOSAVE) ---
    def save_session(self):
        data_to_save = {}
        for rid, b in self.bots.items():
            data_to_save[rid] = {
                "identifier": b['n_en'].get(),
                "sheet": b['s_en'].get(),
                "row": b['r_en'].get(),
                "config": b['cfg_dd'].get(),
                "link": b['lnk_dd'].get(),
        ...
        if os.path.exists("session_bots.json"):
            try:
                with open("session_bots.json", "r") as f:
                    ...
                    for rid, info in saved_data.items():
                        self.add_bot_row(saved_info=info)
            except: pass
    ...
    def setup_link_tab(self):
        ...
        ctk.CTkButton(top_bar, text="🔄 REFRESH LIST", width=120, fg_color=self.color_dark_purple, command=self.refresh_link_list).pack(side="right")
    ...
    def setup_config_tab(self):
        ...
        top_bar = ctk.CTkFrame(container, fg_color="transparent"); top_bar.pack(fill="x", pady=(0, 10))
        ctk.CTkButton(top_bar, text="🔄 REFRESH RULES", width=120, fg_color=self.color_dark_purple, command=self.refresh_config_list).pack(side="right")
        ...
        for i, label in enumerate(labels):
            ...
            en.bind("<KeyRelease>", lambda e: self.check_cfg_inputs()); self.cfg_entries[label] = en
    ...
    def lock_logic(self, rid):
        ...
        b['n_en'].configure(text_color="#EF4444" if (is_dup and name) else "white")
        ...
        self.after(100, lambda: self.sync_colors(rid))

    def sync_colors(self, ex_id):
        for rid, b in self.bots.items():
            if rid != ex_id:
                nm = b['n_en'].get().strip()
                is_d = any(nm == ob['n_en'].get().strip() for oi, ob in self.bots.items() if oi != rid)
                b['n_en'].configure(text_color="#EF4444" if (is_d and nm) else "white")
        ...
        for fn in sorted(os.listdir()):
            if fn.startswith("cfg_") and fn.endswith(".json"):
                try:
                    ...
                    c = ctk.CTkFrame(self.cfg_list_frame, fg_color="#1C1C20", border_width=1, border_color=self.color_main); c.pack(fill="x", pady=4, padx=10)
                    det = (
                        f"📄 Konfigurasi File: {fn}\n"
                        ...
                        f"• Batas Maksimal: {d.get('Max')} | Waktu Timeout: {d.get('Timeout (m)')} menit | Waktu Duplikat: {d.get('DupTime (m)')} menit"
                    )
                    ctk.CTkLabel(c, text=det, font=("Arial", 11, "bold"), justify="left", text_color="#E2E8F0").pack(side="left", padx=15, pady=10)
                    ctk.CTkButton(c, text="🗑 Hapus", width=70, height=35, fg_color="#450a0a", hover_color="#7f1d1d", command=lambda f=fn: [os.remove(f), self.refresh_config_list(), self.refresh_all_bot_dropdowns()]).pack(side="right", padx=15)
                except: pass

    # --- DETAIL CONNECT SHEETS (WARNA DISAMAKAN DENGAN RULES ENGINE) ---
    def refresh_link_list(self):
        ...
        for fn in sorted(os.listdir()):
            if fn.startswith("link_") and fn.endswith(".json"):
                try:
                    with open(fn, "r") as f: d = json.load(f)
                    # WARNA DISAMAKAN: border_color=self.color_main (ungu)
                    c = ctk.CTkFrame(self.link_list_frame, fg_color="#1C1C20", border_width=1, border_color=self.color_main); c.pack(fill="x", pady=2, padx=10)
                    det = f"🔗 Kategori Sheet: {d.get('name')}\n🌐 URL Lengkap: {d.get('url')}"
                    ctk.CTkLabel(c, text=det, font=("Arial", 11, "bold"), justify="left", text_color="#E2E8F0").pack(side="left", padx=15, pady=8)
                    ctk.CTkButton(c, text="🗑 Hapus", width=70, height=35, fg_color="#450a0a", hover_color="#7f1d1d", command=lambda f=fn: [os.remove(f), self.refresh_link_list(), self.refresh_all_bot_dropdowns()]).pack(side="right", padx=10)
                except: pass

    def bot_open_ui(self, rid):
        self.save_session(); b = self.bots[rid]; b['b_web'].configure(state="disabled"); b['status_lbl'].configure(text="LAUNCHING...", text_color=self.color_accent)
    ...
    def open_browser_task(self, rid):
        ...
        try:
            ...
            b['driver'].get(url if url.startswith("http") else "https://"+url); b['browser_ready'] = True
            ...
            while b['driver']: time.sleep(1); b['driver'].window_handles
        except:
            b['driver'] = None; b['browser_ready'] = False
            self.after(0, lambda: [b['status_lbl'].configure(text=f"IDLE ➜ {rid}", text_color="#52525B"), b['b_web'].configure(state="normal")])

    def bot_start_ui(self, rid):
        self.save_session(); b = self.bots[rid]; b['is_running'] = True; b['b_start'].configure(state="disabled"); b['b_stop'].configure(state="normal")
        b['status_lbl'].configure(text=f"RUNNING ➜ {b['n_en'].get()}", text_color="#10B981"); self.add_log(f"Bot {b['n_en'].get()} started.", b['n_en'].get())

    def bot_stop_ui(self, rid):
        self.save_session(); b = self.bots[rid]; b['is_running'] = False; b['b_stop'].configure(state="disabled"); b['b_start'].configure(state="normal")
    ...
    def refresh_all_bot_dropdowns(self):
        cf, ln = ["Select"] + sorted([f for f in os.listdir() if f.startswith("cfg_")]), ["Select"] + sorted([f for f in os.listdir() if f.startswith("link_")])
    ...
    def save_cfg_json(self):
        ...
        nm = re.sub(r'[^a-zA-Z0-9_]', '', f"{d['Name Col']}_{d['Nominal Col']}_{d['Username Col']}_{d['Status Col']}")
        with open(f"cfg_{nm}.json", "w") as j: json.dump(d, j, indent=4)
        for e in self.cfg_entries.values(): e.delete(0, 'end')
        self.refresh_config_list(); self.refresh_all_bot_dropdowns()
    ...
    def on_closing(self): self.save_session(); self.destroy(); os._exit(0)
